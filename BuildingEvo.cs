...
{
    public Resourcemanager rss; // Make sure this is assigned in the inspector
    ...
    {
        if (rss.coins >= requiredGold && rss.wood >= requiredWood && rss.stone >= requiredStone && rss.steel >= requiredMetal)
        {
            rss.coins -= requiredGold;
            rss.wood -= requiredWood;
            rss.stone -= requiredStone;
            rss.steel -= requiredMetal;
            buildingLvl += 1;
